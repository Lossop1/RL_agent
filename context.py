...
EMERGENCY_KEEP_MSGS  = 15       # EmergencySnip: how many recent messages to keep
...
SUMMARY_PROMPT = """你是一个上下文压缩助手。请对以下 Agent 对话历史生成结构化摘要。
...
7. 下一步计划
8. 仿真数据解析的中间结论（已查询的 group/field 及其关键数值、跟踪误差、足地接触模式、速度达标情况）
...
- 仿真数据 engine 输出的 group 聚合统计（avg_density、value_range 等）
...
def emergency_snip(messages: list) -> list:
    ...
    for m in reversed(messages[1:]):
        ...
        if isinstance(content, str) and content.startswith(COMPACT_MARKER):
            summary_msg = m
            break
    tail = messages[-EMERGENCY_KEEP_MSGS:]
    del messages[:]
